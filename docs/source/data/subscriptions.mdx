...
The GraphQL spec does not define a specific protocol for sending subscription requests. In Apollo Client, subscriptions can be sent over HTTP via multipart transport with the default terminating link, `HTTPLink`, or via WebSocket.

### Multipart Subscriptions Over HTTP

In order to use Apollo Client with a back-end that supports multipart subscriptions over HTTP (for example, Apollo Router) no additional client configuration is required. Apollo Client will automatically send the required headers with the request if the terminating `HTTPLink` receives a subscription operation.
...
> As mentioned in [Choosing a subscription library](#choosing-a-subscription-library), some servers that communicate via WebSocket use an older library called `subscriptions-transport-ws`. For necessary changes to use that library, see [below](#the-older-subscriptions-transport-ws-library).

## Subscriptions via Multipart HTTP

No additional libraries or configuration are required. Apollo Client add the required headers to your request when the terminating (and default) `HTTPLink` receives a subscription operation at the `uri` specified when initializing the link or Apollo Client instance.
